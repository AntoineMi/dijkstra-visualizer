...
void drawNode(int num) {
    ...
    glBegin(GL_POLYGON);
    for (i = 0; i <= 64; i++) {
        float angle = i * (PI / (64 / 2));
        glVertex2f(x + cos(angle) * 25, y + sin(angle) * 25);
    }
    glEnd();
    ...
    if (nodeArr[num].showCost) {
        glColor3ub(0, 255, 0);
        glRasterPos2i(nodeArr[num].x - 7, nodeArr[num].y + 50);
        sprintf(str, "%d", distances[num]);
        for (c = str; *c != '\0'; c++) {
            glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *c);
        }
    }
...
void drawLegend() {
    ...
    glRasterPos2i(710, 70);
    for (c = "Animation"; *c != '\0'; c++) {
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, *c);
    }
}
